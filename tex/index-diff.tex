...
%DIF DEL /tmp/index-old.tex   Mon Mar  2 10:31:22 2026
%DIF ADD /tmp/index-new.tex   Mon Mar  2 10:31:37 2026
...
\DIFaddbegin \subsection{\DIFadd{Algorithm Complexity Analysis}}\label{sec:complexity}

\DIFadd{The computational cost of ML-DSA is governed by operations over the polynomial ring $R_q = \mathbb{Z}_q[X]/(X^{256}+1)$ with $q = 8{,}380{,}417$. Let $n = 256$ denote the polynomial degree, and let $(k, \ell)$ denote the matrix dimensions for a given parameter set. The following analysis derives operation counts and asymptotic complexities for each algorithmic component based on FIPS 204~\mbox{%DIFAUXCMD
\cite{NIST-FIPS-204}}\hskip0pt%DIFAUXCMD
. To distinguish NIST-standardized procedures from paper-specific routines, all standard references are denoted as }\emph{\DIFadd{FIPS 204, Algorithm $x$}}\DIFadd{.
}

\subsubsection{\DIFadd{Number Theoretic Transform}}

\DIFadd{The NTT converts polynomial multiplication from $O(n^2)$ schoolbook complexity to $O(n \log n)$ by mapping polynomials from $R_q$ to the NTT algebra $T_q \cong \mathbb{Z}_q^{256}$. The forward NTT (}\emph{\DIFadd{FIPS 204, Algorithm 41}}\DIFadd{) consists of $\log_2 n = 8$ butterfly layers, where layer $i$ performs $2^i$ groups of $n/2^{i+1}$ butterfly operations. Each butterfly requires one modular multiplication and two modular additions, yielding exactly $n/2 \cdot \log_2 n = 1{,}024$ multiplications and $n \log_2 n = 2{,}048$ additions per transform. The inverse NTT (}\emph{\DIFadd{FIPS 204, Algorithm 42}}\DIFadd{) has identical structure with an additional $O(n)$ scaling by $n^{-1} \bmod q = 8{,}347{,}681$. Both transforms execute in $O(n \log n)$ field operations.
}

\DIFadd{Pointwise NTT multiplication (}\emph{\DIFadd{FIPS 204, Algorithm 45}}\DIFadd{) computes $\hat{h}[i] = \hat{f}[i] \cdot \hat{g}[i]$ for $i \in \{0, \ldots, n-1\}$, requiring exactly $n = 256$ modular multiplications in $O(n)$ time. Each modular multiplication employs Barrett reduction, replacing division by precomputed fixed-point reciprocal computation: $x \bmod q \approx x - \lfloor x \cdot \lfloor 2^k / q \rfloor / 2^k \rfloor \cdot q$, where $k = 48$, achieving constant-time execution in two multiply-shift operations.
}

\subsubsection{\DIFadd{Sampling Operations}}

\DIFadd{Matrix expansion (}\textsc{\DIFadd{ExpandA}}\DIFadd{, }\emph{\DIFadd{FIPS 204, Algorithm 32}}\DIFadd{) generates the public matrix $\hat{\mathbf{A}} \in T_q^{k \times \ell}$ by invoking $k \cdot \ell$ instances of rejection sampling from SHAKE-128 output. Each polynomial requires 256 accepted coefficients from 3-byte candidates, with acceptance probability $q / 2^{23} \approx 0.9986$, yielding an expected $\lceil 256 / 0.9986 \rceil \approx 257$ candidates per polynomial. Total cost is $O(k \ell n)$ hash output bytes.
}

\DIFadd{Secret vector sampling (}\textsc{\DIFadd{ExpandS}}\DIFadd{, }\emph{\DIFadd{FIPS 204, Algorithm 33}}\DIFadd{) generates vectors $\mathbf{s}_1 \in R_q^\ell$ and $\mathbf{s}_2 \in R_q^k$ with coefficients bounded by $\eta$ via rejection from SHAKE-256 half-bytes. Each polynomial requires 256 accepted coefficients, with acceptance probability $5/8$ for $\eta = 2$ and $9/16$ for $\eta = 4$. Total cost is $O((k + \ell) n)$ hash operations.
}

\DIFadd{Challenge sampling (}\textsc{\DIFadd{SampleInBall}}\DIFadd{, }\emph{\DIFadd{FIPS 204, Algorithm 29}}\DIFadd{) produces a sparse polynomial $c$ with exactly $\tau$ nonzero coefficients in $\{-1, +1\}$ via a Fisher-Yates permutation driven by SHAKE-256, executing in $O(\tau)$ time with $\tau \in \{39, 49, 60\}$.
}

\subsubsection{\DIFadd{Core Protocol Complexity}}

\DIFadd{Table~\ref{tab:complexity} summarizes the asymptotic complexity of each core operation.
}

\begin{table}[h]
	\centering
	\caption{\DIFaddFL{ML-DSA Algorithm Complexity}}
	\label{tab:complexity}
	\begin{tabular}{lcc}
		\toprule
		\textbf{\DIFaddFL{Operation}}                        & \textbf{\DIFaddFL{Dominant Cost}}                            & \textbf{\DIFaddFL{Complexity}}              \\
		\midrule
		\textsc{\DIFaddFL{ExpandA}}                          & \DIFaddFL{SHAKE-128 sampling                                }& \DIFaddFL{$O(k \ell n)$                    }\\
		\DIFaddFL{NTT / NTT$^{-1}$                          }& \DIFaddFL{Butterfly multiplications                         }& \DIFaddFL{$O(n \log n)$                    }\\
		\DIFaddFL{$\hat{\mathbf{A}} \cdot \hat{\mathbf{v}}$ }& \DIFaddFL{$k$ dot products of $\ell$ NTT polys              }& \DIFaddFL{$O(k \ell n)$                    }\\
		\textsc{\DIFaddFL{Decompose}}                        & \DIFaddFL{Constant-time Barrett division                    }& \DIFaddFL{$O(n)$                           }\\
		\DIFaddFL{Bit-packing                               }& \DIFaddFL{Shift and mask                                    }& \DIFaddFL{$O(n)$                           }\\
		\midrule
		\textbf{\DIFaddFL{KeyGen}}                           & \DIFaddFL{$\hat{\mathbf{A}} \cdot \text{NTT}(\mathbf{s}_1)$ }& \DIFaddFL{$O(k \ell n)$                    }\\
		\textbf{\DIFaddFL{Sign}}                             & \DIFaddFL{Rejection loop $\times$ matrix--vector product    }& \DIFaddFL{$O(\bar{\kappa} \cdot k \ell n)$ }\\
		\textbf{\DIFaddFL{Verify}}                           & \DIFaddFL{$\hat{\mathbf{A}} \cdot \hat{\mathbf{z}}$         }& \DIFaddFL{$O(k \ell n)$                    }\\
		\bottomrule
	\end{tabular}
\end{table}

\textbf{\DIFadd{Key generation}} \DIFadd{(}\emph{\DIFadd{FIPS 204, Algorithm 6}}\DIFadd{) performs }\textsc{\DIFadd{ExpandA}} \DIFadd{in $O(k\ell n)$, }\textsc{\DIFadd{ExpandS}} \DIFadd{in $O((k+\ell)n)$, $\ell$ forward NTTs in $O(\ell n \log n)$, one matrix--vector product in $O(k\ell n)$, $k$ inverse NTTs in $O(kn \log n)$, and }\textsc{\DIFadd{Power2Round}} \DIFadd{over $kn$ coefficients. The matrix--vector product dominates, yielding overall $O(k\ell n)$ complexity.
}

\textbf{\DIFadd{Signing}} \DIFadd{(}\emph{\DIFadd{FIPS 204, Algorithm 7}}\DIFadd{) executes a rejection sampling loop with expected iteration count $\bar{\kappa}$, where $\bar{\kappa} \in \{4.25, 5.1, 3.85\}$ for ML-DSA-44, ML-DSA-65, and ML-DSA-87 respectively. Each iteration performs mask expansion in $O(\ell n)$, $\ell$ forward NTTs in $O(\ell n \log n)$, one matrix--vector product in $O(k\ell n)$, $k$ inverse NTTs in $O(k n \log n)$, decomposition over $kn$ coefficients, challenge hashing and }\textsc{\DIFadd{SampleInBall}} \DIFadd{in $O(\tau)$, one challenge NTT in $O(n \log n)$, three NTT-domain vector--scalar multiplications over $(k + \ell)$ polynomials in $O((k+\ell)n)$, and two vector inverse NTTs in $O((k+\ell)n \log n)$. The matrix--vector product dominates each iteration, yielding $O(\bar{\kappa} \cdot k\ell n)$ total complexity. Rejection sampling introduces timing variability, as each failed iteration repeats the full computational cost.
}

\textbf{\DIFadd{Verification}} \DIFadd{(}\emph{\DIFadd{FIPS 204, Algorithm 8}}\DIFadd{) executes deterministically: }\textsc{\DIFadd{SampleInBall}} \DIFadd{in $O(\tau)$, two forward NTTs in $O((k + \ell) n \log n)$, one matrix--vector product in $O(k\ell n)$, one NTT-domain vector product in $O(kn)$, one vector inverse NTT in $O(kn \log n)$, and }\textsc{\DIFadd{UseHint}} \DIFadd{over $kn$ coefficients. The matrix--vector product dominates at $O(k\ell n)$.
}

\subsubsection{\DIFadd{Parameter-Specific Operation Counts}}

\DIFadd{Table~\ref{tab:op-counts} provides concrete operation counts for each parameter set. Modular multiplications serve as the primary cost metric, as each requires a 64-bit product followed by Barrett reduction.
}

\begin{table}[h]
	\centering
	\caption{\DIFaddFL{Dominant Modular Multiplication Counts per Operation}}
	\label{tab:op-counts}
	\begin{tabular}{lccc}
		\toprule
		\textbf{\DIFaddFL{Component}}                                    & \textbf{\DIFaddFL{DSA-44}}    & \textbf{\DIFaddFL{DSA-65}}    & \textbf{\DIFaddFL{DSA-87}}    \\
		                                                      & \DIFaddFL{$(k{=}4,\ell{=}4)$ }& \DIFaddFL{$(k{=}6,\ell{=}5)$ }& \DIFaddFL{$(k{=}8,\ell{=}7)$ }\\
		\midrule
		\DIFaddFL{NTT (per polynomial)                                  }& \DIFaddFL{1,024              }& \DIFaddFL{1,024              }& \DIFaddFL{1,024              }\\
		\DIFaddFL{$\hat{\mathbf{A}} \cdot \hat{\mathbf{v}}$ (pointwise) }& \DIFaddFL{4,096              }& \DIFaddFL{7,680              }& \DIFaddFL{14,336             }\\
		\DIFaddFL{KeyGen NTTs                                           }& \DIFaddFL{8,192              }& \DIFaddFL{11,264             }& \DIFaddFL{15,360             }\\
		\DIFaddFL{Sign NTTs (per iter.)                                 }& \DIFaddFL{12,288             }& \DIFaddFL{16,384             }& \DIFaddFL{22,528             }\\
		\DIFaddFL{Verify NTTs                                           }& \DIFaddFL{10,240             }& \DIFaddFL{14,336             }& \DIFaddFL{19,456             }\\
		\midrule
		\DIFaddFL{KeyGen total                                          }& \DIFaddFL{12,288             }& \DIFaddFL{18,944             }& \DIFaddFL{29,696             }\\
		\DIFaddFL{Sign total (per iter.)                                }& \DIFaddFL{16,384             }& \DIFaddFL{24,064             }& \DIFaddFL{36,864             }\\
		\DIFaddFL{Sign total (expected)                                 }& \DIFaddFL{69,632             }& \DIFaddFL{122,726            }& \DIFaddFL{141,926            }\\
		\DIFaddFL{Verify total                                          }& \DIFaddFL{14,336             }& \DIFaddFL{22,016             }& \DIFaddFL{33,792             }\\
		\bottomrule
	\end{tabular}
\end{table}


\DIFaddend \subsection{MQTT Protocol}
...
Kim and Seo~\cite{Kim2025} demonstrate that post-quantum signatures introduce prohibitive MQTT authentication overhead, motivating KEM-based architectures. Their CRYSTALS-Kyber implementation achieves 4.32-second handshake completion on 8-bit AVR microcontrollers. Barrett multiplication techniques achieve 1.38--1.51$\times$ performance improvements on ARM Cortex-M3~\cite{Barrett2023}. However, empirical studies evaluating ML-DSA performance within full MQTT protocol implementations remain limited, representing a knowledge gap in post-quantum IoT deployment.

Recent MQTT-specific evaluations further characterize migration costs. Samandari and Gritti~\cite{Samandari2023} analyze post-quantum authentication in MQTT using Dilithium and report latency and payload expansion relative to classical baselines. Rampazzo and Henriques~\cite{Rampazzo2023} quantify hybrid post-quantum overhead in MQTT, showing that hybrid handshakes and signature bundles increase end-to-end latency and message sizes even in short publish-subscribe exchanges.

Major transfer issues for post-quantum migration are increasingly framed as crypto-agility and deployment coordination problems. Large-scale traffic inspection indicates substantial lag in quantum-readiness and emphasizes hybrid key exchange, certificate lifecycle adjustments, and transition monitoring as prerequisites for migration~\cite{Cho2025}. Performance studies in TLS and SSH further show that handshake latency and certificate size overheads remain the dominant integration barriers, with operational trade-offs between interoperability and security assurances~\cite{Kampanakis2020,Sikeridis2020}. These observations align with IoT constraints where bandwidth and energy budgets magnify protocol-level overhead.
